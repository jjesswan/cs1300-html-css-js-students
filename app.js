...
const updatePage = async () => {
  ...
  const highSugarArray = fruitsArray.filter((item) => {
    return item.nutritions.sugar > 15;
  })

  const itemArray = highSugarArray.map((item) => {
    return item.name;
  })
  ...
  fruitElement.innerHTML = "Fruits with sugar over 15 " + itemArray;


  // TODO: Append your new element to the page
  const existingElement = document.getElementById('cs1300-gallery');
  //fruitElement.forEach(addToExisting())
  existingElement.append(fruitElement);

  // function addToExisting(item){
  //   existingElement.append(item);
  // }
